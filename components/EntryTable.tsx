    ...
    <>
      <section className="card">
        <div className="flex flex-wrap items-center gap-3">
          <input value={q} onChange={(e) => setQ(e.target.value)} type="text" placeholder="검색: 항목/내용/담당자" className="input flex-1 min-w-[240px]" />
          <select value={fReflect} onChange={(e) => setFReflect(e.target.value)} className="input">
            <option value="">모두</option><option value="미반영">미반영</option><option value="검토중">검토중</option><option value="반영완료">반영완료</option>
          </select>
          <label className="inline-flex items-center gap-2"><input type="checkbox" className="scale-110" checked={fApp} onChange={(e) => setFApp(e.target.checked)} /><span>APP만</span></label>
          <label className="inline-flex items-center gap-2"><input type="checkbox" className="scale-110" checked={fWeb} onChange={(e) => setFWeb(e.target.checked)} /><span>Web만</span></label>
                    ...
                    <>
                      <td>
                        <select className="input" value={r.reflect} onChange={(e) => updateRowLocal(r.id, { reflect: e.target.value })}>
                          <option>미반영</option><option>검토중</option><option>반영완료</option>
                        </select>
                      </td>
                      <td><input value={r.item} onChange={(e) => updateRowLocal(r.id, { item: e.target.value })} className="input w-full" /></td>
                      <td>
                        <label className="inline-flex items-center gap-2 mr-2">
                          <input type="checkbox" checked={r.platform.includes("APP")}
                            onChange={(e) => { const set = new Set(r.platform); e.target.checked ? set.add("APP") : set.delete("APP"); updateRowLocal(r.id, { platform: Array.from(set) as any }); }} />
                          <span>APP</span>
                        </label>
                        <label className="inline-flex items-center gap-2">
                          <input type="checkbox" checked={r.platform.includes("Web")}
                            onChange={(e) => { const set = new Set(r.platform); e.target.checked ? set.add("Web") : set.delete("Web"); updateRowLocal(r.id, { platform: Array.from(set) as any }); }} />
                          <span>Web</span>
                        </label>
                      </td>
